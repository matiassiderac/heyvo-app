...
function Mudanzas() {
  const { mudanzas, cargandoMudanzas, pedirMudanza } = useDemo();
  ...
  const pedirTurno = () => {
    setEnviando(true);
    void pedirMudanza({ tipo, fecha, franja })
      .then((m) => toast.success(`Turno aprobado. Código ${m.codigo ?? "—"}.`))
      .catch(() => toast.error("No pudimos registrar el turno."))
